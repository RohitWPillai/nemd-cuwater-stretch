#!/bin/bash
# submit.sh -- submit one Cu/water-channel LAMMPS measurement to Cirrus as a batch job.
#
...
#     cd slip        && ../submit.sh slip.in -var eps_sl 0.010
...
rm -f cuw_*.profile cuw_*.dat cuw_*.txt cuw_*.png cuw_*.xyz cuw_*.lammpstrj log.lammps

printf -v LMP_ARGS ' %q' "$@"   # input + any -var args, shell-quoted so spaces/globs survive

JOBID=$(sbatch --parsable <<EOF
#!/bin/bash
#SBATCH --job-name=cuwchan
